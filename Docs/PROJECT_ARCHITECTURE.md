...
├── Config/
│   ├── DefaultEngine.ini                # Core engine: Metal4, Lumen, Nanite, VSM, TSR, Jolt, MassAI, WP, EOS
...
├── Source/
│   ├── GuangzhouOpenWorld.Target.cs     # Game target (arm64, Metal4, UMA, SoLoud)
│   ├── GuangzhouOpenWorldEditor.Target.cs # Editor target
│   └── GuangzhouOpenWorld/
│       ├── GuangzhouOpenWorld.Build.cs  # Module: 29 dependencies + 5 Mac frameworks
...
│       ├── Audio/                       # Audio
│       │   └── GZAudioManager.h/.cpp    # 4 environments, 3D spatial, HRTF, Core Audio
│       └── Network/                     # Networking
│           └── GZNetworkManager.h/.cpp  # EOS sessions, EAC anti-cheat, 64 players
├── Shaders/
│   └── LiquidGlass.usf                  # Metal 4 glass shader (iOS27 refraction standard)
├── Scripts/
│   └── build-mac.sh                     # M1/M2/M3 auto-detect build pipeline
├── Plugins/
│   └── README.md                        # Plugin manifest
├── Docs/
│   ├── PROJECT_ARCHITECTURE.md          # This file
│   └── 4A_STANDARDS_CHECKLIST.md        # 4A quality standards verification
├── Content/
│   └── Maps/                            # Game levels
...
│   └── Chaos                   # Chaos physics engine (Jolt via plugin)
│
├── [AI & Navigation]
│   ├── AIModule                # AI framework
│   ├── NavigationSystem        # Recast NavMesh
...
|-----------|---------|
| Metal | GPU rendering |
| MetalKit | Metal view management |
| CoreAudio | Audio I/O |
| AudioToolbox | Audio processing |
| GameController | Controller input |
...
- Compiler: `arm64` with `-mcpu=apple-m1 -mtune=apple-m1 -march=armv8.5-a`
- Defines: `METAL_4_0=1`, `APPLE_SILICON_UMA=1`, `SOLOUD_ENABLED=1`

**Editor Target** (`GuangzhouOpenWorldEditor.Target.cs`):
- `TargetType.Editor` with `BuildSettingsVersion.V5`
- Standard editor configuration, no extra flags

### 3.2 Build Pipeline

```
build-mac.sh
  ├── [1] Chip Detection (M1/M2/M3)
  ├── [2] UE5.8 Path Discovery
  ├── [3] Generate Xcode Project
  ├── [4] Build Editor (Shipping, arm64)
  ├── [5] Cook Content (Metal 4 Shaders)
  ├── [6] Package .app (Shipping, PAK, compressed)
  ├── [7] Zstd Level 22 Compression
  ├── [8] Create DMG
  └── [9] Verify Binary + Size Report
```

### 3.3 Shader Compilation

- **Shader Model**: Metal 4 (MSL 4.0)
- **Shader Compiler**: Metal Shader Compiler (built into UE5.8)
- **Custom Shaders**: `LiquidGlass.usf` (iOS27 physical refraction standard)
- **Shader Features**:
  - Lumen GI (software ray tracing, 4 bounces)
  - Nanite virtual geometry (compute raster)
...
|---------|-------|-------------|
| RHI Backend | Metal 4 (rhi_Metal) | Native Apple Silicon GPU |
...
| Mesh Shader Rasterization | Enabled |
| Shade Bundle Mode | 1 |
...
- **Engine**: Jolt Physics (via plugin)
...
- **Agent Count**: 8,000+ simultaneous
...
- **Zstd Level 22**: Post-build PAK recompression
...
- **Secondary**: SoLoud (advanced 3D spatial)
...
Runtime detection via `sysctl machdep.cpu.brand_string`:

| Chip | Architecture | Arch Flag | GPU Cores | Neural Engine |
|------|-------------|-----------|-----------|---------------|
| M1 | armv8.5-a | apple-m1 | 8 | 16-core |
| M2 | armv8.5-a | apple-m2 | 10 | 16-core |
| M3 | armv8.6-a | apple-m3 | 10 | 16-core |
...
- Oodle compression: Level 4
- Zstd post-compression: Level 22 (ultra)
...
1. **Metal 4 Only**: No OpenGL/Vulkan fallback. Apple Silicon is the only target.
2. **Software Ray Tracing**: Lumen uses software tracing (signed distance fields) instead of hardware RT, which is not available on Apple Silicon.
3. **Jolt Physics**: Better performance than PhysX on ARM, with deterministic simulation.
4. **World Partition**: 128m grid with independent per-cell lighting, no pre-baked lightmaps.
5. **EOS Bubble Sessions**: Dynamic session architecture for seamless open-world multiplayer.
6. **SoLoud + Core Audio**: Dual audio backend for high-quality 3D spatial audio.
7. **Zstd Level 22**: Post-build PAK recompression for minimal distribution size.
8. **No Gaussian Blur**: All glass/transparency effects use physical refraction (Snell's law), no post-process blur.