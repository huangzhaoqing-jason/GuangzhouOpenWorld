...
|--------|---------|---------|---------------|-------|
| Niagara | UE5.8 | GPU particle VFX | ✅ Native | Metal 4.3 compute shaders |
...
| Substrate | UE5.8 | Material system | ✅ Native | UE5.8 integrated |
...
| Plugin | Version | Purpose | Apple Silicon | Notes |
|--------|---------|---------|---------------|-------|
| **PCGAdvanced** | **v5.8-2.3** | Advanced PCG features | ✅ Native | Extended PCG rule set |
...
| Plugin | Version | Purpose | Apple Silicon | Notes |
|--------|---------|---------|---------------|-------|
| **RecastNavigation** | **v5.8-1.8** | Navigation mesh | ✅ Native | 30Hz update, 3-layer hierarchy |
| MassAI | UE5.8 | Mass AI framework | ✅ Native | 12,000+ agents |
...
| Plugin | Version | Purpose | Apple Silicon | Notes |
|--------|---------|---------|---------------|-------|
| **SoLoud** | **2026** | Audio engine | ✅ Native | Core Audio backend. HRTF, Doppler, 3D spatial. |
...
| **RecastNavigation** | **v5.8-1.8** | MIT | Navigation mesh | ✅ Native | 30Hz, 3-layer hierarchy |
| **SoLoud** | **2026** | zlib | Audio engine | ✅ Native | Core Audio, HRTF, Doppler |
| **OSMImporter** | **v4.2.0** | MIT | OSM data import | ✅ Native | 1:1 Guangzhou import |
| **PCGAdvanced** | **v5.8-2.3** | MIT | Advanced PCG | ✅ Native | Extended rule set |
...
All plugins listed above are **native arm64** and require no Rosetta 2 translation. They are compiled for Apple Silicon with Metal 4.3 and MSL 3.1 support.
...
2. Discovers UE5.8 installation path
3. Generates Xcode 16.x project
4. Builds with Shipping configuration, arm64 only
5. Cooks Metal 4.3 shaders with -ForceMetalShaders
...
|------------|-------------------|-------|
| UE 5.8 | JoltPhysics v6.0.1 | Required for deterministic physics |
| UE 5.8 | RecastNavigation v5.8-1.8 | Required for 3-layer nav hierarchy |
| UE 5.8 | SoLoud 2026 | Required for HRTF + Doppler |
| UE 5.8 | OSMImporter v4.2.0 | Required for 1:1 Guangzhou import |
| UE 5.8 | PCGAdvanced v5.8-2.3 | Required for advanced PCG rules |
...
| Plugin | Reason |
|--------|--------|
| OpenGL | Metal 4.3 only, no OpenGL fallback |
| Vulkan | Metal 4.3 only, no Vulkan fallback |
